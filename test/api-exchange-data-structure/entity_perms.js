...
// ##### GET
var get_object = tests_data.entity_4;
var write_group = tests_data.group_2;
var write_user = tests_data.user_test;

var write_group_perm = tests_data.perms_entity_4_write_group_2
var write_user_perm = tests_data.perms_entity_4_write_user_test

var create_object = tests_data.entity_5;
...
vows.describe('Agenda perms API exchanged data structure').addBatch({
	...
	'CREATE Write Group perm' : {
		topic : function() {
			rest = new Rest();
			rest.post('/api/perms/entity/wg', JSON.stringify({grantTo : write_group.id, applyOn : create_object.id}), this.callback);
		},
		...
		'check id' : function(err, res, data) {
			var data = JSON.parse(data);
			var re = new RegExp("^\/perms/entity\/wg\/[0-9a-z]+$")
	...
	'CREATE Write User perm' : {
		topic : function() {
			rest = new Rest();
			rest.post('/api/perms/entity/wu', JSON.stringify({grantTo : write_user.id, applyOn : create_object.id}), this.callback);
		},
		...
		'check id' : function(err, res, data) {
			var data = JSON.parse(data);
			var re = new RegExp("^\/perms/entity\/wu\/[0-9a-z]+$")
			assert.equal(re.test(data.id), true);
		},
