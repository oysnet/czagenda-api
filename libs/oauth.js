...
exports.generateToken = function (length) {
	...
	for (var i = 0; i < length; i++) {
		var byt = Math.floor(Math.random() * 62);
		if (byt < 26)
			token += String.fromCharCode('a'.charCodeAt(0) + byt);
		else if (byt < 52)
			token += String.fromCharCode('A'.charCodeAt(0) + byt - 26);
		else if (byt < 62)
			token += String.fromCharCode('0'.charCodeAt(0) + byt - 52);
		else
			token += '_';
	}
	return token;
